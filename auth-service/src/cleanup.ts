...
/**
 * Clean up expired password reset tokens for all domains
 * 
 * @param db - D1Database instance
 */
async function cleanupAllPasswordResetTokens(db: D1Database): Promise<void> {
  try {
    const domains = await getActiveDomains(db);
    
    for (const domain of domains) {
      try {
        const cleanedCount = await cleanupExpiredPasswordResetTokens(db, domain);
        if (cleanedCount > 0) {
          console.log(`Cleaned up ${cleanedCount} expired password reset tokens for domain: ${domain}`);
        }
      } catch (error) {
        console.error(`Error cleaning up password reset tokens for domain ${domain}:`, error);
      }
    }
  } catch (error) {
    console.error('Error in password reset token cleanup:', error);
  }
}
